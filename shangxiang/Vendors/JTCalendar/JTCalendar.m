...
//    JTGetAndWriteHoliday *holiday = [[JTGetAndWriteHoliday alloc] init];
//    [holiday getBuddhismHolidayList];
...
- (void)goNextMonth
{
    [self.contentView setContentOffset:CGPointMake((self.contentView.contentOffset.x+self.contentView.frame.size.width) * calendarAppearance.ratioContentMenu, self.contentView.contentOffset.y) animated:NO];
    [self updatePage];
}
...
- (void)updatePage
{
    ...
    NSLog(@"self.contentView.contentOffset.%f pageWidth %f",self.contentView.contentOffset.x,pageWidth);
