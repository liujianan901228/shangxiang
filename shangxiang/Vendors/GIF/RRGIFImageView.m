...
+ (NSMutableArray*)getGifFrames:(NSData*)gifImageData {
	RRGIFImageView* gifImageView = [[RRGIFImageView alloc] initWithGIFData:gifImageData];
	if (!gifImageView) {
		return nil;
	}
...
- (id)initWithGIFData:(NSData*)gifImageData {
	...
	if (![RRGIFImageView isGifImage:gifImageData]) {
...
{
    NSMutableArray * gifFrames = [RRGIFImageView getGifImageFrameFromMem:fileName];

    RRGIFImageView * gifObjView = nil;
    ...
    if (gifFrames==nil) {
        ...
        gifObjView = [[RRGIFImageView alloc] initWithGIFFile:path];
        
        [RRGIFImageView saveGifImageFrameToMem:fileName gif:gifObjView.GIF_frames];

    }else
    {
        gifObjView = [[RRGIFImageView alloc] init];
