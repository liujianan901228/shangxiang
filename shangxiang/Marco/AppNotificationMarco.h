//
//  AppNotificationMarco.h
//  shangxiang
//
//  Created by 倾慕 on 15/1/11.
//  Copyright (c) 2015年 倾慕. All rights reserved.
//

#ifndef shangxiang_AppNotificationMarco_h
#define shangxiang_AppNotificationMarco_h

#define WebLoginNotification @"WebLoginNotificaiton"//微博登录
#define AppUserLoginNotification @"AppUserLoginNotication"//登录
#define AppUserLogoutNotification @"AppUserLogoutNotication"//退出登录
#define ShareSuccessNotification @"ShareNotification"//分享成功


#endif
