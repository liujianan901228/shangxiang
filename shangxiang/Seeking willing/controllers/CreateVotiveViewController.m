//
//  CreateVotiveViewController.m
//  shangxiang
//
//  Created by 倾慕 on 15/2/1.
...
@interface CreateVotiveViewController ()<UITextFieldDelegate,UIPickerViewDelegate>
...
@implementation CreateVotiveViewController
...
- (void)initWithTempleInfo
{
    ...
    [viewHall sd_setImageWithURL:[NSURL URLWithString:_infoObject.templeThumb] placeholderImage:[UIImage imageForKey:@"img_not_available"]];
    ...
    [viewMaster sd_setImageWithURL:[NSURL URLWithString:_infoObject.builddHistThumb] placeholderImage:[UIImage imageForKey:@"img_not_available"]];
    ...
    [labelHall setText:[NSString stringWithFormat:@"%@ (%@)",_infoObject.templeName,_infoObject.province]];
    ...
    [labelMaster setText:_infoObject.builddhistName];
...
//    [_scrollView addSubview:buttonSelectContent];
    ...
    [_commonTextFiled setTextPlaceHolder:@"请输入还愿内容"];
    ...
    UILabel* labelPositionDesirer = [[UILabel alloc] initWithFrame:CGRectMake(_scrollView.width - 20 - _fieldOtherDesirer.width, _commonTextFiled.bottom + 10, 100, 20)];
    ...
    _fieldPositionDesirer = [[UITextField alloc] initWithFrame:CGRectMake(_scrollView.width - 20 - _fieldOtherDesirer.width, labelPositionDesirer.bottom + 5, 130, 35)];
    ...
    UIButton* buttonSubmitCreateOrder = [[UIButton alloc] initWithFrame:CGRectMake(20, _fieldTimeDesirer.bottom + 20, self.view.width - 40, 40)];
...
- (void)requestChoiceInfo:(void(^)(BOOL success)) block
{
    typeof(self) weakSelf = self;
    [weakSelf showChrysanthemumHUD:YES];
    [ListTempleManager getChoiceInfo:[LUtility getWishType:_infoObject.wishType] successBlock:^(id obj) {
        [weakSelf removeAllHUDViews:YES];
